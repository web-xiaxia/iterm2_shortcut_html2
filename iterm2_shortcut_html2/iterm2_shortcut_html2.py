...
async def main(connection: Connection):
    ...
    # 存储信息
    system_config: SystemStorageData = SystemStorageData(main_home)
    storage_data: StorageData = StorageData(system_config)
    #
    py_api: PyApi = PyApi(app, connection)
    exec_api: ExecApi = ExecApi(app, connection, storage_data, py_api)
