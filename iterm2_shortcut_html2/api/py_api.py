...
class PyApi:
    ...
    async def exec_shell(self, shell_text: str) -> Tuple[bool, str]:
        env = os.environ.copy()
        env["PATH"] = f"/usr/local/bin:/opt/homebrew/bin:{os.path.expanduser('~/bin')}:{env['PATH']}"
        print(env)
        shell_pre = await self.storage_data.get_shell_pre()
        p = subprocess.Popen(f"""
{shell_pre}
{shell_text}
""", shell=True, stdout=subprocess.PIPE, executable="/bin/zsh", start_new_session=True, env=env)  # 执行shell语句并定义输出格式
        try:
            while p.poll() is None:  # 判断进程是否结束（Popen.poll()用于检查子进程（命令）是否已经执行结束，没结束返回None，结束后返回状态码）
                if p.wait(timeout=20) != 0:  # 判断是否执行成功（Popen.wait()等待子进程结束，并返回状态码；如果设置并且在timeout指定的秒数之后进程还没有结束，将会抛出一个TimeoutExpired异常。）
